...
setwd(wd)
#RAW TABLE WITH BARCODE USED AS A COLUMN
#d <- read.csv(file="metadata_combined_H3K27me3_H3K27ac.csv")
d <- read.csv(file="H3K27ac_metadata_merged.csv")
#d <- read.csv(file="metadata_merged_DEC23_Barnyard_H3K27me3.csv")
#d <- read.csv(file="metadata_merged_R_fixed_nanoscope_15JULY2024_ac.csv")
```
...
```{r}
filtered_df <- d[d$passedMB == TRUE & d$is_any_cell_barcode == TRUE & d$passed_filters >= 100 & d$all_unique_MB>= 1000,]
```
...
Is it even possible to aggregate the cellranger classifications? When combining the reads, we would have to calculate a new classification based on the ratios.
...
```{r}
#DISPLAY THE UNIQUE BARCODES IN A DATAFRAME
...
The Guassian mixture model (GMM) is a machine learning method used to determine the probability that each data point belongs in a given cluster. As opposed to k-means clustering, GMM is a soft clustering method which means in addition to classifying it gives a measure of how much a data point is associated with each cluster.
...
mm_hist <- function(data, classification) {
  classification_sym <- sym(classification)  # Convert classification column name to a symbol
...
All degrees of freedom (nu) seem to give similar criteria scores.
...
d.fil_skew <- smsn.mix(d.fil$ratio, nu = 1, g = 3, group = TRUE, family = "Skew.normal", obs.prob = TRUE)

d.fil$mix_class <- 'unknown'
d.fil$mix_class[is.finite(d.fil$ratio)] <- as.character(d.fil_skew$group)
d.fil$mix_class <- as.factor(d.fil$mix_class)
```
```{r}
# assigning the classes to the right group based on low, middle, and high means.
component_means <- d.fil_skew$mu
...
d.fil$mix_class <- 'unknown'
d.fil$mix_class[is.finite(d.fil$ratio)] <- class_mapping[d.fil_skew$group]
d.fil$mix_class <- as.factor(d.fil$mix_class)
```
...
```
```{r}
d.fil_probs <- d.fil_skew$obs.prob
```
...
```{r}
d.aggr_probs <- data.frame(d.aggr_skew$obs.prob)
colnames(d.aggr_probs) <- class_mapping
hist(d.aggr_probs$collision, breaks = 100)
hist(d.aggr_probs$human, breaks = 100)
hist(d.aggr_probs$mouse, breaks = 100)
```

# Classification 1: percentage cutoff - 90%

```{r}
#when having more than 90% of the unique reads, called it a single cell. Otherwise a collision
d.fil <- d.fil %>%
...
plot.stacked <- function(df, datatype) {
  df_long <- pivot_longer(df, cols = colnames(df)[3:4], names_to = "classification_type", values_to = "class")
